...
#define FILE BASE_DIR "/file"
#define FILE_TARGET DIR "/renamed/"
...
{
	int fd = CHECK(open(path, O_CREAT | O_RDWR, 0644));
	CHECK(close(fd));
}
...
FN_TEST(rename_non_dir_to_trailing_slash_path)
{
	ensure_test_tree();

	TEST_ERRNO(rename(FILE, FILE_TARGET), ENOTDIR);
	TEST_SUCC(access(FILE, F_OK));
	TEST_ERRNO(access(FILE_TARGET, F_OK), ENOENT);
